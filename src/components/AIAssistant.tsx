    ...
    <div className="fixed inset-y-0 right-0 z-50 w-full sm:w-[450px] bg-stone-900 border-l border-stone-800 shadow-2xl flex flex-col justify-between animate-slide-in text-left">
      ...
      <div className="flex justify-between items-center p-4 bg-stone-950 border-b border-stone-850">
        <div className="flex items-center gap-2">
          <div className="p-2 bg-emerald-800/20 border border-emerald-500/20 text-[color:var(--color-gold)] rounded-xl">
          ...
          <div>
            ...
            <span className="text-[10px] text-stone-500 uppercase tracking-wider flex items-center gap-1">
              <Sparkles className="w-3 h-3 text-[color:var(--color-gold)]" /> Gemini 3.5-Flash Active
            </span>
          </div>
        </div>
      ...
      <div className="flex-grow p-4 overflow-y-auto space-y-4 bg-stone-950/40">
        {messages.map((m, idx) => (
          ...
          >
            ...
            <div className={`p-2 rounded-xl h-fit flex-shrink-0 ${
              m.role === "user" ? "bg-stone-800 text-[color:var(--color-gold)]" : "bg-emerald-700 text-white keep-white"
            }`}>
        ...
        {loading && (
          <div className="flex gap-3 max-w-[85%] mr-auto">
            <div className="p-2 bg-emerald-700 text-white keep-white rounded-xl h-fit">
      ...
      <form onSubmit={handleSendMessage} className="p-4 bg-stone-950 border-t border-stone-850 flex gap-2">
        <input
          ...
          className="flex-grow px-4 py-3 bg-stone-900 border border-stone-850 focus:border-emerald-700 focus:ring-1 focus:ring-emerald-700 rounded-xl text-white placeholder-stone-600 text-xs sm:text-sm outline-none"
        />
        <button
          type="submit"
          disabled={loading || !inputText.trim()}
          className="p-3 bg-emerald-700 hover:bg-emerald-800 disabled:opacity-50 text-white keep-white rounded-xl transition-all cursor-pointer"
        >
