...
-- Migration 0014: per-locale video source (uploaded file vs YouTube link)
alter table public.products
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'products_video_source_en_check') then
    alter table public.products add constraint products_video_source_en_check check (video_source_en in ('upload', 'youtube'));
  end if;
  if not exists (select 1 from pg_constraint where conname = 'products_video_source_fr_check') then
